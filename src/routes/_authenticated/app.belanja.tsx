...
import { Plus, Trash2, ShoppingCart, CalendarClock, LayoutTemplate, Loader2, AlertTriangle, Check, ChevronsUpDown } from "lucide-react";
...
function BelanjaPage() {
  ...
  if (isLoading) {
    return (<><PageHeader title="Belanja Bulan Ini" /><div className="space-y-3">{[0,1,2].map(i=><Skeleton key={i} className="h-16" />)}</div></>);
  }

  return (
    <>
      <PageHeader title="Belanja Bulan Ini" subtitle={`Daftar kebutuhan ${namaBulan(bulan)} ${tahun}`} action={
        belanja && <AddItemDialog belanjaId={belanja.id} userId={userId!} onDone={refetch} />
      } />

      {/* Budget warning */}
      {budget > 0 && ratio >= 0.8 && (
        <div className={`mb-4 flex items-center gap-2 rounded-xl border p-3 text-sm ${ratio > 1 ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-warning/30 bg-warning/10 text-warning"}`}>
      ...
      {items.length === 0 ? (
        <EmptyState icon={<ShoppingCart className="h-7 w-7" />} title="Daftar belanjamu masih kosong"
          desc="Yuk mulai susun kebutuhan bulan ini — tambah manual, ambil dari bulan lalu, atau pakai template kategori."
          action={
            <div className="flex flex-wrap justify-center gap-2">
              {belanja && <AddItemDialog belanjaId={belanja.id} userId={userId!} onDone={refetch} />}
              <Link to="/app/generate" search={{ mode: "bulan-lalu" }}><Button variant="outline"><CalendarClock className="mr-2 h-4 w-4" /> Ambil dari Bulan Lalu</Button></Link>
              <Link to="/app/generate" search={{ mode: "template" }}><Button variant="outline"><LayoutTemplate className="mr-2 h-4 w-4" /> Template Kategori</Button></Link>
            </div>
          } />
      ) : (
        <div className="space-y-6">
          <div className="flex flex-wrap gap-2">
            <Link to="/app/generate" search={{ mode: "bulan-lalu" }}><Button variant="outline" size="sm"><CalendarClock className="mr-2 h-4 w-4" /> Ambil dari Bulan Lalu</Button></Link>
            <Link to="/app/generate" search={{ mode: "template" }}><Button variant="outline" size="sm"><LayoutTemplate className="mr-2 h-4 w-4" /> Template Kategori</Button></Link>
          </div>

          {[...grouped.entries()].map(([katId, rows]) => {
            const subtotal = rows.reduce((s, i) => s + Number(i.estimasi_harga) * Number(i.jumlah), 0);
            return (
              <div key={katId} className="overflow-hidden rounded-2xl border bg-card shadow-sm">
                <div className="flex items-center justify-between border-b bg-muted/40 px-4 py-2.5">
                  <span className="font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">{katNama.get(katId) ?? "Lainnya"}</span>
                  <span className="font-mono text-xs text-muted-foreground">{formatRupiah(subtotal)}</span>
                </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button size="lg"><ShoppingCart className="mr-2 h-4 w-4" /> Mulai Belanja</Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Mulai belanja sekarang?</AlertDialogTitle>
                    <AlertDialogDescription>
                      Daftar akan masuk mode belanja. Kamu bisa centang item & catat harga aktual saat di toko.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
...
function AddItemDialog({ belanjaId, userId, onDone }: { belanjaId: string; userId: string; onDone: () => void }) {
  ...
  const { data: master } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("item").select("id, nama, kategori_barang_id, satuan_default").order("nama");
      return data ?? [];
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) reset(); }}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle>Tambah Item Belanja</DialogTitle></DialogHeader>
        <div className="space-y-4">
              ...
              <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                <Command>
                  ...
                  <CommandList>
                    ...
                    <CommandGroup>
                      {(master ?? []).map((m) => (
                        <CommandItem key={m.id} value={m.nama} onSelect={() => {
                          setSelectedItem({ id: m.id, nama: m.nama, kategori_barang_id: m.kategori_barang_id, satuan: m.satuan_default });
                          setSatuan(m.satuan_default); setManualNama(""); setPickerOpen(false);
                        }}>
                          <Check className={`mr-2 h-4 w-4 ${selectedItem?.id === m.id ? "opacity-100" : "opacity-0"}`} />
                          {m.nama}
                        </CommandItem>
                      ))}
                    </CommandGroup>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>Jumlah</Label>
              <Input type="number" min={0.1} step={0.1} value={jumlah} onChange={(e) => setJumlah(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Satuan</Label>
