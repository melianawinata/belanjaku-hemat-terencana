...
import { Wallet, Calculator, Receipt, PiggyBank, TrendingUp, TrendingDown, Repeat, ShoppingCart, ListPlus } from "lucide-react";
...
function Dashboard() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: belanja } = await supabase
        .from("belanja_bulanan").select("*")
        .eq("user_id", userId!).eq("bulan", bulan).eq("tahun", tahun).maybeSingle();

      let items: BelanjaItemRow[] = [];
      if (belanja) {
        const { data: it } = await supabase.from("belanja_item").select("*").eq("belanja_id", belanja.id);
  ...
  const deltas = [...latestByItem.values()].filter((e) => e.prev !== undefined && e.prev !== e.harga)
    .map((e) => ({ nama: e.nama, delta: ((e.harga - e.prev!) / e.prev!) * 100 }));
  const naik = deltas.filter((d) => d.delta > 0).sort((a, b) => b.delta - a.delta).slice(0, 3);
  const turun = deltas.filter((d) => d.delta < 0).sort((a, b) => a.delta - b.delta).slice(0, 3);
  ...
  return (
    <>
      <PageHeader title="Dashboard" subtitle={`Ringkasan belanja ${namaBulan(bulan)} ${tahun}`} action={
        <div className="flex gap-2">
          <Link to="/app/belanja"><Button variant="outline"><ListPlus className="mr-2 h-4 w-4" /> Susun Daftar</Button></Link>
          <Link to="/app/belanja"><Button><ShoppingCart className="mr-2 h-4 w-4" /> Mulai Belanja</Button></Link>
        </div>
      } />
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border bg-card p-5 shadow-sm">
              ...
              {pieData.length === 0 ? <p className="py-12 text-center text-sm text-muted-foreground">Belum ada data.</p> : (
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={90} paddingAngle={2}>
                      {pieData.map((_, i) => <Cell key={i} fill={CHART[i % CHART.length]} />)}
                    </Pie>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <InsightCard title="Barang Naik Harga" icon={<TrendingUp className="h-4 w-4 text-destructive" />}
              rows={naik.map((d) => ({ nama: d.nama, badge: `▲ ${d.delta.toFixed(0)}%`, tone: "danger" as const }))} empty="Belum ada kenaikan harga." />
            <InsightCard title="Barang Turun Harga" icon={<TrendingDown className="h-4 w-4 text-success" />}
              rows={turun.map((d) => ({ nama: d.nama, badge: `▼ ${Math.abs(d.delta).toFixed(0)}%`, tone: "success" as const }))} empty="Belum ada penurunan harga." />
            <InsightCard title="Paling Sering Dibeli" icon={<Repeat className="h-4 w-4 text-info" />}
              rows={sering.map(([nama, n]) => ({ nama, badge: `${n}x`, tone: "info" as const }))} empty="Belum ada data pembelian." />
          </div>
        </div>
      )}
    </>
...
function InsightCard({ title, icon, rows, empty }: {
  title: string; icon: React.ReactNode;
  rows: { nama: string; badge: string; tone: "danger" | "success" | "info" }[]; empty: string;
}) {
  const toneClass = { danger: "text-destructive bg-destructive/10", success: "text-success bg-success/10", info: "text-info bg-info/10" };
  return (
    <div className="rounded-2xl border bg-card p-5 shadow-sm">
      <div className="mb-3 flex items-center gap-2"><span>{icon}</span><p className="text-sm font-semibold">{title}</p></div>
      {rows.length === 0 ? <p className="py-6 text-center text-xs text-muted-foreground">{empty}</p> : (
        <ul className="space-y-2">
          {rows.map((r, i) => (
            <li key={i} className="flex items-center justify-between text-sm">
              <span className="truncate">{r.nama}</span>
              <span className={`ml-2 shrink-0 rounded-full px-2 py-0.5 font-mono text-xs ${toneClass[r.tone]}`}>{r.badge}</span>
            </li>
          ))}
        </ul>
      )}
