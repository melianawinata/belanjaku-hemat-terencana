...
interface Row { item_id: string | null; nama: string; kategori_barang_id: string | null; jumlah: number; satuan: string; checked: boolean }
...
function GeneratePage() {
    ...
    queryFn: async () => {
      if (mode === "bulan-lalu") {
        ...
        const { data: items } = await supabase.from("belanja_item")
          .select("item_id, nama_snapshot, kategori_barang_id, jumlah, satuan").eq("belanja_id", pb.id);
        return (items ?? []).map((i) => ({
          item_id: i.item_id, nama: i.nama_snapshot, kategori_barang_id: i.kategori_barang_id,
        ...
        return (defs ?? []).map((d) => d.item).filter(Boolean).map((it) => {
          ...
          return { item_id: item.id, nama: item.nama, kategori_barang_id: item.kategori_barang_id, jumlah: 1, satuan: item.satuan_default, checked: true };
        }) as Row[];
      }
    },
  });
  ...
  const tambah = async () => {
    ...
    const { data: existing } = await supabase.from("belanja_item").select("id, item_id, nama_snapshot, jumlah").eq("belanja_id", belanja.id);
    const existMap = new Map((existing ?? []).map((e) => [e.item_id ?? e.nama_snapshot, e]));

    for (const r of chosen) {
      const key = r.item_id ?? r.nama;
      const found = existMap.get(key);
      if (found) {
        await supabase.from("belanja_item").update({ jumlah: Number(found.jumlah) + r.jumlah }).eq("id", found.id);
      } else {
        const estimasi = await getEstimasiHarga(userId, r.item_id);
        await supabase.from("belanja_item").insert({
          belanja_id: belanja.id, user_id: userId, item_id: r.item_id, nama_snapshot: r.nama,
            ...
            <ul className="divide-y">
              {rows.map((r, idx) => (
                <li key={idx} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm font-medium">{r.nama}</span>
