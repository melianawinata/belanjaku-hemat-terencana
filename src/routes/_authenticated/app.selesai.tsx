...
function SelesaiPage() {
  ...
  const simpan = async () => {
    ...
    // harga_aktual = total per baris, jadi histori disimpan per-satuan (÷ jumlah)
    // supaya estimasi bulan depan (yang per-satuan) tidak dobel.
    const historiRows = dibeli
      .filter((i) => i.item_id && i.harga_aktual != null)
      .map((i) => ({
        item_id: i.item_id!,
        user_id: userId!,
        harga: Math.round(Number(i.harga_aktual) / (Number(i.jumlah) || 1)),
        merk: i.merk,
        toko_id: i.toko_id,
    ...
    await logActivity(
      userId!,
      "selesai",
      `Selesai belanja ${namaBulan(bulan)} ${tahun} — total ${formatRupiah(realisasi)}`,
    );
  ...
  if (isLoading)
    return (
      <>
        <PageHeader title="Ringkasan Belanja" />
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-xl space-y-5">
        <div
          className={`flex items-center gap-3 rounded-2xl border p-5 ${hemat ? "border-success/30 bg-success/10" : "border-destructive/30 bg-destructive/10"}`}
        >
          <span
            className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${hemat ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive"}`}
          >
          ...
          <div>
            <p className="text-base font-semibold">
              {hemat
                ? `Mantap, kamu hemat ${formatRupiah(Math.abs(selisih))} bulan ini! 🎉`
                : `Belanja melebihi budget ${formatRupiah(Math.abs(selisih))}.`}
            </p>
            <p className="text-sm text-muted-foreground">
              {hemat
                ? "Terus pertahankan kebiasaan baik ini."
                : "Coba sesuaikan daftar bulan depan biar lebih hemat."}
            </p>
          </div>
        </div>
            ...
            {dibeli.map((i) => (
              <li key={i.id} className="flex items-center justify-between px-4 py-2.5 text-sm">
                <span>
                  {i.nama_snapshot}
                  {i.merk && (
                    <span className="text-xs text-muted-foreground"> ({i.merk})</span>
                  )}{" "}
