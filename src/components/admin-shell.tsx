    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-app-bg px-4 text-center">
        ...
        <p className="max-w-sm text-sm text-muted-foreground">
          Halaman ini khusus admin. Akun kamu tidak punya akses ke area admin.
        </p>
    ...
    <div className="min-h-screen bg-app-bg">
      <aside className="fixed inset-y-0 left-0 hidden w-60 flex-col border-r bg-sidebar lg:flex">
        <div className="flex items-center gap-2 px-5 py-5">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-info text-white">
            <ShoppingBasket className="h-5 w-5" />
          </span>
          <div>
            <span className="font-mono text-sm font-semibold">BelanjaKu</span>
            <p className="font-mono text-[10px] uppercase tracking-wider text-info">Admin</p>
          </div>
        </div>
        <nav className="flex-1 space-y-1 px-3">
          {NAV.map((n) => (
            <Link
              key={n.to}
              to={n.to}
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${isActive(n.to) ? "bg-info/10 text-info" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
        ...
        <div className="space-y-1 border-t p-3">
          <Link
            to="/app/dashboard"
            className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-muted-foreground hover:bg-muted"
          >
            <ArrowLeft className="h-4 w-4" /> Mode Pengguna
          </Link>
          <Button
            variant="ghost"
      ...
      <div className="lg:pl-60">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b bg-background/80 px-4 py-3 backdrop-blur sm:px-6">
          <div className="flex items-center gap-2">
            <Link
              to="/app/dashboard"
              aria-label="Kembali ke Mode Pengguna"
          ...
          <Link
            ...
            title="Profil Saya"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-info/15 font-mono text-sm font-semibold text-info transition-colors hover:bg-info/25"
          >
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex overflow-x-auto border-t bg-background lg:hidden">
        {NAV.map((n) => (
          <Link
            key={n.to}
            to={n.to}
            className={`flex min-w-[64px] flex-1 flex-col items-center gap-0.5 py-2 text-[9px] ${isActive(n.to) ? "text-info" : "text-muted-foreground"}`}
          >
